 ...
 */
public class MeetStarter extends AbstractQuest {

	private static final String QUEST_SLOT = "meet_starter";

	private static final int TIME_OUT = 60;

	private static Logger logger = Logger.getLogger(MeetStarter.class);
	...
	public List<String> getHistory(final Player player) {
		...
		res.add("Starter is the first guy I ever met in this world " );
		if (player.getQuest(QUEST_SLOT, 0).equals("start") 
		&& new KilledForQuestCondition(QUEST_SLOT,1).fire(player, null, null)) {
			res.add("I have met that rat, I should go back to tell him!");
		}
		if (player.getQuest(QUEST_SLOT, 0).equals("start")) {
...
//		}
	...
	private void prepareStarter() {

		final SpeakerNPC npc = npcs.get("Starter");
		...
		npc.add(
				...
				"Programming task are interesting. "
				+" #UweSusi needs help on her tasks."
				+"You can talk with Professors #UweDatatype and #UweComparsion at Lab. " +
				"They are very knowledgeable and they will tell you about DataType and Comparison. " +
				"  ",
				null);
		
		npc.add(
				ConversationStates.INFORMATION_1,
				Arrays.asList("UweDatatype", "DataType"),
				new AndCondition(new GreetingMatchesNameCondition(getName()),
						new QuestInStateCondition(QUEST_SLOT, 0, "start")),
				ConversationStates.INFORMATION_1,
				" Datatype is a important topic in learning prgoramming. To find out more, Go talk with #UweDataType at XXXX." +
				" He can give you a lot information about DataType."
				 ,
				null);

		npc.add(
				ConversationStates.INFORMATION_1,
				Arrays.asList("UweComparsion", "Comparsion"),
				new AndCondition(new GreetingMatchesNameCondition(getName()),
						new QuestInStateCondition(QUEST_SLOT, 0, "start")),
				ConversationStates.INFORMATION_1,
				" Go talk with #UweComparsion at XXXX." +
				" He can show you information about comparsion operators ."
				 ,
				null);
		
		npc.add(
				ConversationStates.INFORMATION_1,
				Arrays.asList("UweSusi", "Susi"),
				new AndCondition(new GreetingMatchesNameCondition(getName()),
						new QuestInStateCondition(QUEST_SLOT, 0, "start")),
				ConversationStates.INFORMATION_1,
				"UweSusi at XXX needs help on progrmmming exercises."
				 ,
				null);
		...
		npc.add(
				...
				"Fine, you seem like an intelligent type. You will figure it out.",
				null);
		
	final List<ChatAction> reward = new LinkedList<ChatAction>();
	reward.add(new EquipItemAction("money", 50000));
	reward.add(new EquipItemAction("="));
	reward.add(new EquipItemAction("hint1"));
	...
	npc.add(ConversationStates.ATTENDING, 
	 ...
	 /*
		...
		npc.addReply(
				 "Factter" ,
				 "Factter is stated at the library and go and check it out."
				 );
...
//			new MultipleActions(reward));
//
//	   	// increase player didn't finish learning everything when he came after killing the rat, he must have another chance. Here it is.
...
//				null);
	...
	public void addToWorld() {
		fillQuestInfo(
				"Meet Starter",
				"Starter an teach young heroes important basics of the world.",
				false);
		prepareStarter();
	}
