package games.stendhal.server.maps.quests.uwe.conditions;
 ...
 * @author yoriy
 */
@Dev(category=Category.MEETS, label="Meets?")
public class MetForQuestCondition implements ChatCondition {
	private static Logger logger = Logger.getLogger(MetForQuestCondition.class);
	 ...
	 */
	@Dev
	public MetForQuestCondition(String quest, @Dev(defaultValue="1") int index) {
	...
	public boolean fire(final Player player, final Sentence sentence, final Entity npc) {
		...
		for (int i = 0; i < tokens.size() / 5; i++) {
			...
			final int diffSolo = player.getSoloMeet(creatureName) - killedSolo - toKillSolo;
			final int diffShared = player.getSharedMeet(creatureName) - killedShared - toKillShared;
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof MetForQuestCondition)) {
			return false;
		}
		MetForQuestCondition other = (MetForQuestCondition) obj;
