 ...
 * @author hendrik
 */
public final class QuestionListImageViewerEvent extends ViewPanel {
	 ...
	 * @param event event
	 */
	public QuestionListImageViewerEvent(RPEvent event) {
		this.event = event;
	...
	private Object[] createDataRow(RPObject item, int maxPreferredWidth) {
		...
		rval[1] = getFormattedSymbol(item);
	 ...
	 * @return html code to display price
	 */
	private String getFormattedSymbol(RPObject item) {
		String price = "";
		if (item.has("name")) {
			String symbol = item.get("name");
			price = "<html><span style=\"color: ";
			if (symbol != null) {
				price = price + "#FFFFFF\">";
			} else {
